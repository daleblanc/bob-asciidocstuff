...
MongoDB is a NoSQL, document-centric database. MongoDB is intended to used small in enterprise-grade projects that need a document-centric database solution that scales up on demand.
...
The following query combines the `apples` and `countries` collections together. The `apple` collection is combined with the `countries` collection by using the `apple.countryOfOrigin` field, and the data in the `countries.country` as the common join fields.
